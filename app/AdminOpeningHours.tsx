...
function normalize(value: any): Record<string, DayConfig> {
  ...
  return Object.fromEntries(
    DAYS.map(([id]) => [id, { ...DEFAULT, ...(source[id] || {}) }]),
  );
}

export default function AdminOpeningHours() {
  const [host, setHost] = useState<HTMLElement | null>(null);
  const [hours, setHours] = useState<Record<string, DayConfig>>(() =>
    normalize({}),
  );
  ...
  const tenant = useMemo(
    () =>
      typeof window === "undefined"
        ? "sabor-da-casa"
        : new URLSearchParams(location.search).get("restaurant") ||
          "sabor-da-casa",
    [],
  );
  ...
  useEffect(() => {
    ...
    if (!token) return;
    api(`admin?restaurant=${encodeURIComponent(tenant)}`, {
  ...
  async function save() {
    try {
      ...
      setMessage("");
      const token = localStorage.getItem("mesaflow-token") || "";
      await api(`settings?restaurant=${encodeURIComponent(tenant)}`, {
        method: "PATCH",
        headers: { authorization: `Bearer ${token}` },
        body: JSON.stringify({ opening_hours: hours, timezone }),
      });
      setMessage(
        "✓ Horários salvos. A loja agora bloqueia pedidos quando estiver fechada.",
      );
  ...
  if (!host) return null;
  return createPortal(
    <section className="settings-card opening-hours-card">
      <div className="settings-title">
        <div>
          <span className="muted-caps">FUNCIONAMENTO</span>
          <h2>Horários da loja</h2>
          <p>
            Fora desses horários, o checkout é bloqueado também no servidor.
          </p>
        </div>
      </div>
      <div className="opening-hours-list">
        {DAYS.map(([id, label]) => {
          const d = hours[id] || DEFAULT;
          return (
      ...
      <label className="opening-hours-timezone">
        Fuso horário
      ...
      <div className="payment-method-actions">
        <small>
          {message ||
            "Horário vazio mantém a compatibilidade com lojas já existentes."}
        </small>
