...
export default function AdminPaymentMethods(){
 ...
 async function save(){if(!methods.length){setMessage('Selecione pelo menos um meio de pagamento.');return}try{setSaving(true);setMessage('');const token=localStorage.getItem('mesaflow-token')||'';await api(`settings?restaurant=${tenant}`,{method:'PATCH',headers:{authorization:`Bearer ${token}`},body:JSON.stringify({payment_methods:methods})});setMessage('✓ Meios de pagamento salvos.');window.dispatchEvent(new Event('mesaflow:settings-saved'))}catch(e:any){setMessage(e.message)}finally{setSaving(false)}}
 if(!host)return null;
 return createPortal(<section className="settings-card payment-methods-card"><div className="settings-title"><div><span className="muted-caps">PAGAMENTO</span><h2>Meios aceitos</h2><p>Escolha exatamente o que o cliente poderá usar no checkout.</p></div></div>{groups.map(g=><div className="payment-group" key={g.title}><b>{g.title}</b><div className="payment-method-grid">{g.items.map(([id,label])=><label className={`payment-method ${methods.includes(id)?'selected':''}`} key={id}><input type="checkbox" checked={methods.includes(id)} onChange={()=>toggle(id)}/><span><i>{methods.includes(id)?'✓':''}</i>{label}</span></label>)}</div></div>)}<div className={`payment-method-actions ${message?'has-message':''}`}><small role="status">{message||`${methods.length} selecionado${methods.length===1?'':'s'}`}</small><button className="button button-dark" onClick={save} disabled={saving}>{saving?'Salvando…':'Salvar meios'}</button></div></section>,host)
}
