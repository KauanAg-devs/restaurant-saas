...
function AdminPanel(){
 ...
 const logout=()=>{localStorage.removeItem("mesaflow-token");setToken("");setData(null)};
 ...
 if(!data)return null;
 return <div className="admin-shell"><aside className="admin-sidebar"><div className="admin-brand"><span className="admin-mark">M</span><div><b>MesaFlow</b><small>Restaurant OS</small></div></div><div className="tenant-chip sidebar-tenant-chip"><span>{data.restaurant.name?.[0]}</span><div><b>{data.restaurant.name}</b><small>{data.role==="owner"?"Proprietário":"Equipe"}</small></div></div><nav>{ADMIN_TABS.map(item=><button className={tab===item.id?"selected":""} key={item.id} onClick={()=>router.push(adminUrl(tenant,item.id))}>{item.label}</button>)}</nav><div className="sidebar-bottom"><button onClick={logout}>Sair</button></div></aside><main className="admin-content"><header><div className="admin-heading"><span className="muted-caps">{data.restaurant.name}</span><h1>{ADMIN_TABS.find(item=>item.id===tab)?.label}</h1><p>{tab==="appearance"?"Defina a identidade visual da loja sem complicação.":tab==="catalog"?"Crie produtos, envie fotos e controle a disponibilidade.":tab==="categories"?"Crie, organize e controle as categorias do cardápio.":tab==="settings"?"Configure operação, entrega, pagamentos e atendimento.":"Gerencie esta área da sua operação."}</p></div><div className="admin-header-actions"><a className="admin-open-store" href={`/loja/${tenant}`} target="_blank" rel="noreferrer"><span aria-hidden="true">↗</span>Abrir loja</a><button className="mobile-profile-chip" onClick={logout} title="Sair"><span>{data.restaurant.name?.[0]}</span><span><b>{data.restaurant.name}</b><small>{data.role==="owner"?"Proprietário":"Equipe"}</small></span></button></div></header>
