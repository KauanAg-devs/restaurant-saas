...
function AdminPanel(){
 const router=useRouter(),searchParams=useSearchParams(),accountRef=useRef<HTMLDivElement>(null);
 ...
 const [token,setToken]=useState(""),[sessionReady,setSessionReady]=useState(false),[email,setEmail]=useState(""),[password,setPassword]=useState(""),[data,setData]=useState<AdminData|null>(null),[error,setError]=useState(""),[loading,setLoading]=useState(false),[accountOpen,setAccountOpen]=useState(false);
 ...
 useEffect(()=>{if(!accountOpen)return;const close=(e:PointerEvent)=>{if(accountRef.current&&!accountRef.current.contains(e.target as Node))setAccountOpen(false)};document.addEventListener("pointerdown",close);return()=>document.removeEventListener("pointerdown",close)},[accountOpen]);
 ...
 const logout=()=>{localStorage.removeItem("mesaflow-token");setToken("");setData(null);setAccountOpen(false)};
 ...
 if(!data)return null;
 return <div className="admin-shell"><aside className="admin-sidebar"><div className="admin-brand"><span className="admin-mark">M</span><div><b>MesaFlow</b><small>Restaurant OS</small></div></div><div className="tenant-chip sidebar-tenant-chip"><span>{data.restaurant.name?.[0]}</span><div><b>{data.restaurant.name}</b><small>{data.role==="owner"?"Proprietário":"Equipe"}</small></div></div><div className="mobile-account" ref={accountRef}><button className="mobile-account-trigger" aria-label="Abrir menu da conta" aria-expanded={accountOpen} onClick={()=>setAccountOpen(v=>!v)}><span className="mobile-account-icon" aria-hidden="true">♙</span><span className="mobile-account-copy"><b>{data.restaurant.name}</b><small>Conta</small></span><span className="mobile-account-chevron" aria-hidden="true">⌄</span></button>{accountOpen?<div className="mobile-account-menu"><div><b>{data.restaurant.name}</b><small>{data.role==="owner"?"Proprietário":"Equipe"}</small></div><button onClick={()=>{setAccountOpen(false);router.push(adminUrl(tenant,"settings"))}}>Conta e configurações</button><button className="mobile-account-logout" onClick={logout}>Sair</button></div>:null}</div><nav>{ADMIN_TABS.map(item=><button className={tab===item.id?"selected":""} key={item.id} onClick={()=>router.push(adminUrl(tenant,item.id))}>{item.label}</button>)}</nav><div className="sidebar-bottom"><button onClick={logout}>Sair</button></div></aside><main className="admin-content"><header><div className="admin-heading"><span className="muted-caps">{data.restaurant.name}</span><h1>{ADMIN_TABS.find(item=>item.id===tab)?.label}</h1><p>{tab==="appearance"?"Defina a identidade visual da loja sem complicação.":tab==="catalog"?"Crie produtos, envie fotos e controle a disponibilidade.":tab==="categories"?"Crie, organize e controle as categorias do cardápio.":tab==="settings"?"Configure operação, entrega, pagamentos e atendimento.":"Gerencie esta área da sua operação."}</p></div><div className="admin-header-actions"><a className="admin-open-store" href={`/loja/${tenant}`} target="_blank" rel="noreferrer"><span aria-hidden="true">↗</span>Abrir loja</a></div></header>
