...
export default function StorePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="store" style={{'--brand':r.primary_color,'--brand2':r.secondary_color,'--store-bg':r.background_color} as React.CSSProperties}>
    <header className="store-nav"><div className="store-wrap"><div className="store-brand"><div className="store-logo">{r.logo_url?<img src={r.logo_url} alt=""/>:r.name?.[0]}</div><div><strong>{r.name}</strong><small>● Aberto agora</small></div></div><button className="store-cart" onClick={()=>setDrawer(true)}>Sacola · {cart.reduce((s,i)=>s+i.quantity,0)}</button></div></header>
