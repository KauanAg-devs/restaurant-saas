...
export default function StorePage({
  ...
  useEffect(() => {
    ...
    setHasAdminSession(Boolean(localStorage.getItem("mesaflow-token")));
  }, [slug]);
  useEffect(() => {
    if (slug)
  ...
  const products = useMemo(
    () =>
      data?.products.filter(
        (p) => category === "all" || p.category_id === category,
      ) ?? [],
    [data, category],
  );
  const paymentMethods = useMemo(() => {
    const methods = data?.restaurant?.payment_methods;
    return Array.isArray(methods)
      ? methods.filter((m: any) => typeof m === "string" && m.trim())
      : [];
  }, [data]);
  useEffect(() => {
    if (paymentMethods.length && !paymentMethods.includes(form.payment_method))
      setForm((f) => ({ ...f, payment_method: paymentMethods[0] }));
    if (!paymentMethods.length && form.payment_method)
  ...
  const subtotal = cart.reduce(
    (sum, item) =>
      sum +
      (item.price + item.addons.reduce((s, a) => s + Number(a.price), 0)) *
        item.quantity,
    0,
  );
  const deliveryFee =
    form.fulfillment === "retirada"
      ? 0
      : Number(data?.restaurant?.delivery_fee || 0);
  const total = subtotal + deliveryFee;
  function addSelected() {
    if (!selected) return;
    const addons = (selected.addons || []).filter((a: any) =>
  ...
  async function submitOrder() {
    if (!form.customer_name.trim() || !form.customer_phone.trim())
      return setError("Preencha nome e telefone para continuar.");
    if (
      ...
      return setError("Preencha rua, número e bairro para entrega.");
    if (!paymentMethods.length)
      return setError(
        "Este restaurante ainda não configurou meios de pagamento.",
      );
    if (!paymentMethods.includes(form.payment_method))
      return setError("Escolha uma forma de pagamento disponível.");
    try {
      setSending(true);
      setError("");
      const result: any = await api("order", {
        ...
        body: JSON.stringify({
          ...
          fulfillment_type:
            form.fulfillment === "entrega" ? "delivery" : "pickup",
          street: form.street,
      ...
      setConfirmedOrder({
        ...
        items: cart.map((item) => ({
          ...
          total:
            ...
            item.quantity,
          addons: item.addons.map((addon) => String(addon.name)),
        })),
        subtotal,
        deliveryFee,
        ...
        fulfillment: form.fulfillment === "entrega" ? "delivery" : "pickup",
        address:
          form.fulfillment === "entrega"
            ? [form.street, form.street_number, form.neighborhood]
                .filter(Boolean)
                .join(", ")
            : "",
        paymentLabel:
  ...
  if (error && !data)
    return (
      <main className="store-state">
        <div>
          <span className="eyebrow">MESAFLOW</span>
  ...
  const theme = {
    ...
    "--store-bg": r.background_color || "#fff9f1",
    "--surface": r.surface_color || "#ffffff",
    "--text": r.text_color || "#171717",
    "--muted": r.muted_text_color || "#777777",
    "--radius": `${r.border_radius || 20}px`,
    "--store-font": r.font_family || "system-ui",
  } as React.CSSProperties;
  const heroStyle = r.hero_style || "gradient";
  const buttonStyle = r.button_style || "solid";
  const heroInline =
    heroStyle === "image" && r.hero_image_url
      ? {
          backgroundImage: `linear-gradient(#0007,#0007),url(${r.hero_image_url})`,
        }
      : undefined;
  const whatsappUrl = confirmedOrder
    ? buildOrderWhatsAppUrl(r.whatsapp, confirmedOrder)
    : "";
  return (
    <div
      className={`store hero-${heroStyle} button-${buttonStyle}`}
      style={theme}
    >
          ...
          <div className="store-actions">
            {hasAdminSession ? (
              <a
                ...
                href={`/admin?restaurant=${encodeURIComponent(slug)}`}
              >
                <span className="shortcut-icon">
                  <Settings size={14} aria-hidden="true" />
                </span>
                <span className="shortcut-label">Painel</span>
              </a>
            ) : null}
            <button
      ...
      <main className="store-wrap">
        <section className="store-hero" style={heroInline}>
          <div>
            ...
            <b>
              {r.delivery_minutes_min}–{r.delivery_minutes_max} min · Entrega{" "}
              {money(r.delivery_fee)}
            </b>
          </div>
          <div className="hero-dish">
            {r.logo_url ? <img src={r.logo_url} alt="" /> : "🍽️"}
          </div>
        </section>
          ...
          {products.map((p) => (
            ...
            >
              ...
              <div className="product-copy">
                <small>{p.featured ? "MAIS PEDIDO" : "DISPONÍVEL"}</small>
                <h3>{p.name}</h3>
                <p>{p.description}</p>
                <footer>
                  <strong>{money(p.price)}</strong>
                  <button className="themed-button">Escolher</button>
                </footer>
              </div>
            </article>
          ))}
      ...
      {selected ? (
        <div className="overlay" onClick={() => setSelected(null)}>
          ...
          >
            ...
            <div
              className={`modal-art ${selected.image_url ? "has-image" : ""}`}
            >
              {selected.image_url ? (
            ...
            <span className="eyebrow">PERSONALIZE SEU PEDIDO</span>
            <h2>{selected.name}</h2>
            ...
            <div className="modal-bottom">
              ...
              <button className="button button-dark" onClick={addSelected}>
                Adicionar ·{" "}
                {money(
                  (Number(selected.price) +
                    (selected.addons || [])
                      .filter((a: any) => selectedAddons.includes(a.id))
                      .reduce((s: number, a: any) => s + Number(a.price), 0)) *
                    quantity,
                )}
              </button>
            </div>
            ...
            {cart.length ? (
              <>
                <div className="cart-items">
                  {cart.map((item, i) => (
                    <div className="cart-line" key={`${item.product_id}-${i}`}>
                      <div>
                        <b>
                ...
                <div className="cart-summary">
                  ...
                  <span>
                    Entrega <b>{money(Number(r.delivery_fee))}</b>
                  </span>
                </div>
                <button
              ...
              <div className="drawer-empty">
                <span>
                  <ShoppingBag size={46} strokeWidth={1.5} aria-hidden="true" />
                </span>
                <h3>Sua sacola está vazia</h3>
                <p>Escolha algo gostoso no cardápio.</p>
              </div>
            )}
          </aside>
        </div>
              ...
              <label>
                ...
                {paymentMethods.length ? (
                  ...
                  <div className="errorBox">
                    Restaurante sem meios de pagamento configurados.
                  </div>
                )}
              </label>
              {form.fulfillment === "entrega" ? (
      ...
      {confirmedOrder ? (
        <div className="overlay">
          <section className="success-modal">
            <div>✓</div>
            <span className="muted-caps">PEDIDO RECEBIDO</span>
            <h2>Pedido #{confirmedOrder.number}</h2>
            <p>
              Seu pedido já foi registrado.{" "}
              {whatsappUrl
                ? "Agora envie o resumo ao restaurante pelo WhatsApp."
                : "O restaurante acompanhará pelo painel."}
            </p>
            ...
            {whatsappUrl ? (
              <small className="success-whatsapp-note">
