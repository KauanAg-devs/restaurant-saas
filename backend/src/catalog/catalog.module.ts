...
class CatalogController {
  constructor(
    ...
    private categories: Repository<CategoryEntity>,
    @InjectRepository(ProductEntity)
    private products: Repository<ProductEntity>,
    @InjectRepository(ProductAddonEntity)
    private addons: Repository<ProductAddonEntity>,
  ) {}
  @Get() async get(@Query("restaurant") slug = "sabor-da-casa") {
    ...
    const addonRows = products.length
      ? await this.addons
    ...
    return {
      restaurant: {
        ...
        is_open: isOpen(restaurant.openingHours, restaurant.timezone),
      },
      categories: categories.map(categoryResponse),
      products: products.map((p) =>
        productResponse(
          p,
          addonRows.filter((a) => a.productId === p.id),
        ),
      ),
    };
  }
...
function isOpen(hours: Record<string, any>, tz: string) {
  ...
  const current = Number(hh) * 60 + Number(mm);
  const slot =
    hours[day] ||
    hours[
  ...
  return (
    current >= toMin(slot.open || slot.from) &&
    current <= toMin(slot.close || slot.to)
  );
}
