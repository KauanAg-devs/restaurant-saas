...
import { buildOrderWhatsAppMessage, buildOrderWhatsAppUrl, normalizeWhatsAppNumber } from '../lib/whatsapp-order.mjs';

const order = {
  number: '1234', restaurantName: 'Sabor da Casa',
  items: [{ quantity: 2, name: 'Burger', total: 59.8, addons: ['Bacon', 'Queijo'] }],
  subtotal: 59.8, deliveryFee: 5, total: 64.8,
  customerName: 'Ana', customerPhone: '(11) 98888-7777', fulfillment: 'delivery',
  address: 'Rua A, 10, Centro', paymentLabel: 'Pix', notes: 'Sem cebola',
};
...
test('omits delivery-only and optional lines for pickup', () => {
  const message = buildOrderWhatsAppMessage({ ...order, fulfillment: 'pickup', address: '', notes: '', deliveryFee: 0 });
